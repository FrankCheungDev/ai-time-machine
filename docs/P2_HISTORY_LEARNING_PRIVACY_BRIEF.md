...
| 2012      | AlexNet            | 论文在约 120 万训练图像、1000 类任务上使用深层 CNN 与 GPU 实现并显著降低错误率       | [ImageNet Classification with Deep CNNs](https://papers.nips.cc/paper_files/paper/2012/hash/c399862d3b9d6b76c8436e924a68c45b-Abstract.html)       | 把算法、数据和算力视为共同转折                             |
...
本地记录仅保存章节 id、首次是否答对、尝试次数、是否打开解释，以及本设备是否建议复习该章。v1.7 的复习建议只由这些本地自测记录确定性生成，不包含能力分数、时间戳或身份字段；记录与界面语言共享，可从首页或任意章节清除全部自测与复习记录，且该操作不影响独立的章节完成状态。
