...
type TimelineChapterId = Exclude<ChapterId, "overview">;
...
type TimelineEntryCopy = Omit<TimelineEntry, "id" | "chapterId">;

const localizedTimelineEntryCopy = {
  "zh-CN": {
    search: {
      era: "规则与搜索",
      year: "1950s-1960s",
  ...
  en: {
    ...
    "decision-boundary": {
      era: "Classical Machine Learning",
      year: "1990s-2000s",
      title: "Classical Machine Learning And Decision Boundaries",
      summary:
...
function hasTimelineEntry(
  ...
  return chapter.id !== "overview" && Boolean(chapter.timelineId);
}

export function getAiTimelineEntries(
  locale: Locale = defaultLocale,
): TimelineEntry[] {
  const localizedCopy = getLocalizedValue(localizedTimelineEntryCopy, locale);

  return chapterRegistry.filter(hasTimelineEntry).map((chapter) => ({
    id: chapter.timelineId,
