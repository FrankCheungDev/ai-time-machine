  ...
  steps: [
    {
      ...
      title: "拆解任务并制定计划",
      loopLabel: "Plan",
      description: "Agent 先把用户目标拆成可执行步骤，决定需要查什么、算什么、再回答什么。",
      activeNodeIds: ["plan"],
      activeEdgeIds: []
    },
    {
      ...
      title: "调用工具获取真实信息",
      loopLabel: "Tool Call",
      description: "它不只生成文字，还会调用搜索、数据库、代码执行器等外部工具获取新状态。",
      activeNodeIds: ["tool"],
    ...
    },
    {
      ...
      description: "工具返回结果后，Agent 判断结果是否足够、是否可信、是否需要换一个路径。",
    ...
    },
    {
      ...
      title: "观察失败并修正计划",
      loopLabel: "Revise",
      description: "如果工具失败或结果不完整，Agent 会调整计划，再决定下一次工具调用或降级策略。",
  ...
  branchOptions: [
    {
      ...
      label: "模拟工具失败",
      targetStepId: "revise",
      description: "工具返回空结果或过期信息，Agent 需要观察失败原因并修正计划。"
    }
  ]
